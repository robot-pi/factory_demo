...
void MTCTaskNode::setupPlanningScene()
{
  ...
  object.primitives[0].dimensions = { 0.015, 0.015, 0.15 };
...
{
  mtc::Task task;
  task.stages()->setName("pick_and_place task");
...
#pragma GCC diagnostic pop
  ...
  std::unordered_map<std::string, std::string> ompl_map_arm = {
	{"ompl",  arm_group_name + std::string("[RRTConnectkConfigDefault]")}
	};

  auto ompl_planner_arm = std::make_shared<mtc::solvers::PipelinePlanner>(node_);

  ompl_planner_arm->setPlannerId(ompl_map_arm["ompl"]);
  ...
  stage_open_hand->setGoal("half_closed");
...
grasp_frame_transform.translation().z() = 0.1;
  ...
  auto stage_move_to_place = std::make_unique<mtc::stages::Connect>(
      ...
      mtc::stages::Connect::GroupPlannerVector{ { arm_group_name, ompl_planner_arm },
                                                { hand_group_name, ompl_planner_arm } });

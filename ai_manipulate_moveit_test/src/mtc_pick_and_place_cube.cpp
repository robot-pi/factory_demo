  ...
  : Node("mtc_node", options)
{
    ...
    declare_parameter("gripper_frame", "panda_link8", "Name of the gripper frame");
    ...
    declare_parameter("world_frame", "panda_link0", "Name of the world frame");

    // Table parameters
    declare_parameter("table_name", "table", "Name of the table in the planning scene");
    declare_parameter("table_reference_frame", "panda_link0", "Reference frame for the table");
    ...
    declare_parameter("object_reference_frame", "panda_link0", "Reference frame for the object");
    declare_parameter("object_dimensions", std::vector<double>{0.05, 0.05, 0.05}, "Dimensions of the object [height, radius]");
    declare_parameter("object_pose", std::vector<double>{0.3, -0.5, 0.0, 1.0}, "Initial pose of the object [x, y, z, roll, pitch, yaw]");
...
void MTCTaskNode::setupPlanningScene()
{
    ...
    // Create a wood_cube collision object
    geometry_msgs::msg::Pose wood_cube_pose = vectorToPose(object_pose_param);
    auto place_pose_z_offset_factor = this->get_parameter("place_pose_z_offset_factor").as_double();
    wood_cube_pose.position.z += place_pose_z_offset_factor * object_dimensions[0]; // Adjust z position before creating the object
    moveit_msgs::msg::CollisionObject wood_cube_object;
    wood_cube_object.id = object_name;
    wood_cube_object.header.frame_id = object_reference_frame;
    wood_cube_object.primitives.resize(1);
    wood_cube_object.primitives[0].type = shape_msgs::msg::SolidPrimitive::BOX;
    wood_cube_object.primitives[0].dimensions = {
        object_dimensions.at(0), object_dimensions.at(1), object_dimensions.at(2) };

    wood_cube_object.primitive_poses.push_back(wood_cube_pose);

    // Add the wood_cube to the planning scene
    if (!psi.applyCollisionObject(wood_cube_object)) {
        RCLCPP_ERROR(this->get_logger(), "Failed to spawn object: %s", wood_cube_object.id.c_str());
        throw std::runtime_error("Failed to spawn object: " + wood_cube_object.id);
    }
...
mtc::Task MTCTaskNode::createTask()
{
    ...
    RCLCPP_INFO(this->get_logger(), "Robot current state: %s", current_state_ptr ? "valid" : "invalid");
    ...
    RCLCPP_INFO(this->get_logger(), "Starting 'open gripper' stage...");
    /****************************************************
     *                                                  *
     *               Move to Pick                       *
     *                                                  *
